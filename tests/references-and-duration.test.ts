...
test("duration selector is fixed, normalizes legacy values, and scene duration overrides auto", () => {
  assert.deepEqual(supportedVideoDurations, [4, 6, 8, 10, 12, 15]); assert.equal(normalizeVideoDuration(5, supportedVideoDurations), 4); assert.equal(normalizeVideoDuration(7.8, supportedVideoDurations), 8);
