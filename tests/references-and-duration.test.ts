...
test("duration selector is fixed, normalizes legacy values, and scene duration overrides auto", () => {
  assert.deepEqual(supportedVideoDurations, [4, 6, 8, 10, 12]); assert.equal(normalizeVideoDuration(5, supportedVideoDurations), 4); assert.equal(normalizeVideoDuration(7.8, supportedVideoDurations), 8); assert.equal(normalizeVideoDuration(11, supportedVideoDurations), 10); assert.equal(normalizeVideoDuration(99, supportedVideoDurations), 12);
