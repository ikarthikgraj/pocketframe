...
test("mock audio versions are append-only and approval gates project readiness", async () => {
  ...
  assert.equal(first.versionNumber, 1); assert.equal(second.versionNumber, 2); assert.equal(repo.getProject(project.id)?.status, "SHOT_GENERATION"); assert.equal(repo.approveTts(scene.id)?.status, "TTS_APPROVED"); assert.equal(repo.getProject(project.id)?.status, "SHOT_GENERATION");
