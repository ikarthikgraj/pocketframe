...
test("scene versions increment, retain regeneration and upload history, and approval is exclusive", () => {
  ...
  assert.equal(repo.approveSceneVersion(first.id)?.status, "APPROVED"); assert.equal(repo.approveSceneVersion(second.id)?.status, "APPROVED"); const versions = repo.listSceneVersions(scene.id); assert.equal(versions.find((version) => version.id === first.id)?.status, "READY"); assert.equal(versions.find((version) => version.id === second.id)?.status, "APPROVED"); assert.equal(repo.getScene(scene.id)?.approvedVersionId, second.id); assert.equal(repo.getProject(project.id)?.status, "VOICE_REVIEW");
