...
test("scene versions increment, retain regeneration and upload history, and only the latest review version can be approved", () => {
  ...
  assert.equal(repo.approveSceneVersion(first.id), undefined); assert.equal(repo.approveSceneVersion(second.id), undefined); assert.equal(repo.approveSceneVersion(upload.id)?.status, "APPROVED"); const versions = repo.listSceneVersions(scene.id); assert.equal(versions.find((version) => version.id === first.id)?.status, "READY"); assert.equal(versions.find((version) => version.id === upload.id)?.status, "APPROVED"); assert.equal(repo.getScene(scene.id)?.approvedVersionId, upload.id); assert.equal(repo.getProject(project.id)?.status, "VOICE_REVIEW");
  const rejected = repo.createSceneVersion({ sceneId: scene.id, provider: "manual", prompt: "Rejected replacement", negativePrompt: "Manual", status: "READY" });
  assert.equal(repo.rejectSceneVersion(rejected.id, "Not usable")?.status, "REJECTED"); assert.equal(repo.listSceneVersions(scene.id).find((version) => version.id === rejected.id)?.status, "REJECTED"); database.close();
});
...
test("final render readiness is gated until every scene has exactly one approved version", () => {
  ...
  const second = repo.createSceneVersion({ sceneId: other.id, provider: "manual", prompt: "Two", negativePrompt: "No", status: "READY" }); repo.approveSceneVersion(second.id); assert.equal(repo.getProject(project.id)?.status, "VOICE_REVIEW"); database.close();
});
