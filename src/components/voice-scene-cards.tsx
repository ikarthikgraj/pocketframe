...
export function VoiceSceneCards({ scenes, audioVersions, voiceBible }: Props) {
  ...
  return <section id="voice" className="voice-section"><div className="section-heading"><div><p className="eyebrow">Narration review</p><h2>Voice</h2><p><b>Narration Source:</b> Exact synopsis text</p><p>Visual scenes are planned first. Narration is generated after the visual cut is ready.</p></div></div>{voiceBible && <article className="voice-bible"><div><p className="eyebrow">Voice bible</p><h3>{voiceBible.narratorPersona}</h3><p>{voiceBible.voiceStyle}</p></div><dl><div><dt>Tone</dt><dd>{voiceBible.tone}</dd></div><div><dt>Pace</dt><dd>{voiceBible.baselinePace}</dd></div><div><dt>Pronunciation</dt><dd>{voiceBible.pronunciationNotes}</dd></div><div><dt>Language</dt><dd>{voiceBible.languageCode}</dd></div><div><dt>Provider</dt><dd>{voiceBible.ttsProvider}</dd></div></dl></article>}{error && <p className="error" role="alert">{error}</p>}<div className="scene-list">{scenes.map((scene) => {
...
function VoiceCard({ scene, latest, approved, working, onGenerate, onApprove }: { scene: Scene; latest?: AudioVersion; approved: boolean; working: boolean; onGenerate: (scene: Scene, prompt: string) => void; onApprove: (sceneId: string) => void }) {
  ...
  return <article className={`card voice-card ${approved ? "is-approved" : ""}`}><div className="section-heading"><h3>Scene {String(scene.sceneNumber).padStart(2, "0")}</h3><span className={`status-badge ${approved ? "status-approved" : latest ? "status-generated" : "status-pending"}`}>{approved ? "Approved" : latest ? "Generated" : "Pending"}</span></div><p className="scene-text"><b>Exact narration text</b>{scene.exactText}</p><div className="detail-row"><span><b>Emotion</b>{scene.emotion}</span><span><b>Intensity</b>{scene.intensity}/10</span><span><b>Pace</b>{scene.pace}</span></div><label>Delivery direction<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} disabled={working} /></label>{latest && <audio controls src={`/api/audio-versions/${latest.id}`}><a href={`/api/audio-versions/${latest.id}`}>Play audio</a></audio>}<div className="actions"><button onClick={() => onGenerate(scene, prompt)} disabled={working}>{working ? "Generating narration…" : latest ? "Regenerate" : "Generate Narration"}</button>{latest && <button className="secondary" onClick={() => onApprove(scene.id)} disabled={working || approved}>{approved ? "Approved" : "Approve"}</button>}</div><p className="duration-line"><b>Measured duration:</b> {seconds(scene.ttsDurationMs)} <span /> <b>Final visual duration:</b> {seconds(scene.targetVideoDurationMs)}</p>{versionsLabel(latest)}</article>;
}
