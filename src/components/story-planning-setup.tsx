...
export function StoryPlanningSetup({ projectId, productionBible, scenes, status }: Props) {
  ...
  return <section id="story" className="planning"><div className="section-heading"><div><p className="eyebrow">Production planning</p><h2>Production Bible</h2><p>Approve the editorial story plan, then generate silent visual shots.</p></div>{status === "BIBLE_READY" && <button onClick={() => action("approve-bible")} disabled={working}>{working ? "Approving…" : "Approve Production Bible"}</button>}</div>{error && <p className="error" role="alert">{error}</p>}<article className="bible-summary"><div><span className="field-label">Premise</span><p>{productionBible.premise.text}</p></div><div><span className="field-label">Hook</span><p>{productionBible.hook.text}</p></div><div className="bible-chips"><span>{productionBible.tone.text}</span><span>{productionBible.mood.text}</span>{productionBible.themes.map((theme) => <span key={theme.text}>{theme.text}</span>)}</div><div className="bible-details">{fields.slice(2).map((field) => <div key={field}><span className="field-label">{field === "visualStyle" ? "Visual style" : field}</span><p>{productionBible[field].text}</p><small>{label(productionBible[field].groundedness)}</small></div>)}</div></article><h3 className="group-title">Characters</h3><div className="card-grid">{productionBible.characters.map((character) => <article className="card compact-card" key={character.name}><span className="status-badge status-neutral">{label(character.description.groundedness)}</span><h4>{character.name}</h4><p><b>Visual identity</b> {character.description.text}</p></article>)}</div><h3 className="group-title">Environments</h3><div className="card-grid">{productionBible.environments.map((environment) => <article className="card compact-card environment-card" key={environment.name}><span className="field-label">Environment</span><h4>{environment.name}</h4><p><b>Mood & lighting</b> {environment.description.text}</p></article>)}</div><div className="section-heading scene-plan-heading"><div><h3>Scene plan</h3><p>{productionBible.trailerDurationSeconds}s target runtime</p></div></div><div className="scene-list">{scenes.map((scene) => <article className="card scene-plan-card" key={scene.id}><span className="scene-number">{String(scene.sceneNumber).padStart(2, "0")}</span><div><p className="scene-text">{scene.exactText}</p><div className="detail-row"><span><b>Emotion</b>{scene.emotion}</span><span><b>Camera</b>{scene.cameraIntent}</span><span><b>Estimate</b>{scene.estimatedDurationSeconds}s</span><span><b>Readiness</b>{scene.approvedVersionId ? "Shot approved" : "Visual plan ready"}</span></div></div></article>)}</div></section>;
}
