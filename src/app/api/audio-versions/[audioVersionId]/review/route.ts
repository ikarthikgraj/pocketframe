...
export async function PATCH(request: Request, { params }: Context) {
  ...
  if (!input.success) return NextResponse.json({ error: { code: "VALIDATION_ERROR", message: "Choose select, approve, reject, or unapprove." } }, { status: 400 });
  ...
  if (input.data.action === "unapprove") {
    const version = repo.unapproveAudioVersion(id);
    return version ? NextResponse.json({ version }) : NextResponse.json({ error: { code: "AUDIO_NOT_APPROVED", message: "Only the current approved narration can be unapproved." } }, { status: 409 });
  }
